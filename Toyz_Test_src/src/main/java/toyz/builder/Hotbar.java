...
public final class Hotbar {
    ...
    public static class InventoryState {
        ...
        /** Vertical scroll of the piece grid (pixels). */
    ...
    public static void updateFullInventory(InventoryState state, HotbarState hotbar, List<Piece.PieceDef> defs) {
        ...
        int rows = (indices.size() + columns - 1) / columns;
        float contentH = Math.max(rows * cellH, gridH);
        ...
        if (CheckCollisionPointRec(m, panel)) {
            ...
            if (wheel != 0f) {
                state.scrollY -= wheel * cellH;
            }
        }
        // Clamp scroll
        if (state.scrollY < 0f) state.scrollY = 0f;
        if (state.scrollY > maxScroll) state.scrollY = maxScroll;

        int col = 0;
        float gx = x, gy = gridTop - state.scrollY;

        BeginScissorMode((int) x, (int) gridTop, (int) (panelW - 28), (int) gridH);
        for (int idx : indices) {
            Piece.PieceDef def = defs.get(idx);
            Rectangle cellRect = Helpers.newRectangle(gx + col * cellW, gy, 72, 56);
            // Skip draw if fully outside scissor (cheap)
            if (cellRect.y() + cellRect.height() < gridTop - 4 || cellRect.y() > gridBottom + 4) {
                col++;
                if (col >= columns) { col = 0; gy += cellH; }
                continue;
            }
            // Steel cell
        ...
        // Scrollbar
        if (maxScroll > 1f) {
            float trackX = panel.x() + panelW - 18;
            float trackY = gridTop;
            float trackH = gridH;
            DrawRectangle((int) trackX, (int) trackY, 8, (int) trackH, Helpers.newColor(12, 14, 16, 255));
            float thumbH = Math.max(24f, trackH * (gridH / contentH));
            float thumbY = trackY + (trackH - thumbH) * (state.scrollY / maxScroll);
            DrawRectangle((int) trackX, (int) thumbY, 8, (int) thumbH, UI.phosphorDim());
            // Click track to jump
            Rectangle track = Helpers.newRectangle(trackX, trackY, 8, trackH);
            if (CheckCollisionPointRec(m, track) && IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
                float t = (m.y() - trackY - thumbH * 0.5f) / Math.max(1f, trackH - thumbH);
                if (t < 0) t = 0; if (t > 1) t = 1;
                state.scrollY = t * maxScroll;
            }
        }

        DrawText(indices.size() + " pieces  |  wheel scroll", (int) x, (int) (panel.y() + panelH - 28), 13, UI.phosphorDim());
