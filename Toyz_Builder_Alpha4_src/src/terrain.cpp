...
ForestTerrain GenerateForestTerrain(unsigned int seed) {
    ...
    // Wider spacing on the larger map keeps draw calls reasonable on old GPUs
...
void DrawForestTerrain(const ForestTerrain& forest) {
    ...
    // Leaf palettes (Terasology-inspired greens + autumn accents, still flat-shaded)
    ...
    for (const ForestTree& tree : forest.trees) {
        ...
        if (tree.biomeType == 3) {
            // Low bush — 2 offset spheres, no tall trunk
        ...
        if (tree.biomeType == 1) {
            // Pine — tall thin trunk + stacked conical canopy layers
            DrawModelEx(forest.pineModel,
                {base.x, y0 + 1.2f * s, base.z},
                {0, 1, 0}, tree.rotation, {s * 0.7f, s * 1.35f, s * 0.7f}, Color{75, 48, 28, 255});
            Color pine = kPine[cv];
            float layers[4] = {1.6f, 2.35f, 3.0f, 3.55f};
            float widths[4] = {1.35f, 1.1f, 0.85f, 0.55f};
            for (int i = 0; i < 4; i++) {
                DrawModelEx(forest.canopyModel,
                    {base.x, y0 + layers[i] * s, base.z},
                    {0, 1, 0}, tree.rotation + i * 15.0f,
                    {s * widths[i], s * 0.55f, s * widths[i]},
                    (i == 3) ? Fade(pine, 0.85f) : pine);
            }
            continue;
        }

        // Broadleaf / autumn — trunk + multi-lobe canopy (offset spheres = organic silhouette)
        ...
        // Side lobes for irregular outline (cheap, big visual win)
        DrawModelEx(forest.canopyModel,
        ...
        if (s > 0.9f) {
            DrawModelEx(forest.canopyModel,
                {base.x + 0.1f * s, y0 + 2.7f * s, base.z + 0.4f * s},
                {0, 1, 0}, tree.rotation + 10.0f, {s * 0.9f, s * 0.7f, s * 0.9f}, Fade(leaf, 0.85f));
        }
    }
}
