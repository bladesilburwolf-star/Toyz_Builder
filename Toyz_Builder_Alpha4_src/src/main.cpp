...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        // Sky from preset (works every time — no broken jpg blit)
        ...
        if (!pauseMenu.open) {
            ...
            DrawGameHud(buildMode == BuildMode::MagneticSnap ? "MAGNETIC" : "SHRINE",
                        ...
                        (int)placedPieces.size(), hasController);
        }
        ...
        if (pauseMenu.open) {
            ...
            if (pauseMenu.showOptionsPanel) {
                ...
                Rectangle opt = { sw * 0.5f + 20.0f, sh * 0.5f - 260.0f, 340.0f, 520.0f };
