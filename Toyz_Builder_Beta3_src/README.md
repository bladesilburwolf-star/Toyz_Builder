...
This progress build makes the procedural forest a gameplay space rather than just scenery.

- The spherical builder collides with procedural tree trunks.
- The third-person follow camera avoids tree trunks and terrain clipping where practical.
- Construction rays now hit the procedural terrain rather than an invisible flat Y=0 plane.
- Placed pieces can start at the actual terrain height.
- The selected construction piece is visibly carried in front of the builder while placement mode is active.
- Controller-first placement remains centered on the screen for the gamepad workflow.

Asset convention remains:
...
## Forest Builder v4 — Lincoln Log Construction + Controller Inventory

This version changes the prototype from generic piece placement into the start of a Lincoln Logs construction loop:

- Logs are now horizontal 3m pieces instead of vertical cylinders.
- Structural log layers alternate 90 degrees.
- New logs snap to construction intersections and rise by one log diameter.
- A placement preview is green when the location is available and red when occupied.
- The builder mascot carries the selected construction piece.
- The inventory was rebuilt around controller navigation instead of requiring a mouse.
- Controller layout was simplified:
  - Left Stick = move
  - Right Stick = camera
  - A = jump / place
  - B = cancel build / close inventory
  - X = select a placed piece
  - Y = inventory
  - LB/RB or D-Pad Left/Right = rotate
  - LT/RT = raise/lower selected piece
  - Menu = delete selected piece
- The starter scene contains two crossed foundation logs to demonstrate the alternating-layer rule.
