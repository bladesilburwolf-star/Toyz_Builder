...
static bool FindNearestSnap(Vector3 worldPos, std::vector<PlacedPiece>& placed, const std::vector<PieceDef>& defs, Vector3& outPos) {
    ...
    for (auto& p : placed) {
        ...
        for (auto& d : defs) if (d.type == p.type) {
            if (UsesColorVariants(p.type) && d.defaultColor != p.color) continue;
            defPtr = &d; break;
        }
        if (!defPtr) continue;
        bool nearThis = false;
    ...
    while (!WindowShouldClose()) {
        ...
        if (isPlacing && !menuOpen) {
            ...
            if (buildMode == BuildMode::MagneticSnap && FindNearestSnap(aim, placedPieces, pieceDefs, snappedSpot)) {
                ...
                for (int i = 0; i < (int)placedPieces.size(); i++) {
                    ...
                    for (auto& d : pieceDefs) if (d.type == placedPieces[i].type) {
                        if (UsesColorVariants(placedPieces[i].type) && d.defaultColor != placedPieces[i].color) continue;
                        defPtr = &d; break;
                    }
                    if (!defPtr) continue;
                    for (const auto& sp : defPtr->snapPoints) {
                ...
                if (stackOnIndex >= 0) {
                    ...
                    for (auto& d : pieceDefs) if (d.type == placedPieces[stackOnIndex].type) {
                        if (UsesColorVariants(placedPieces[stackOnIndex].type) && d.defaultColor != placedPieces[stackOnIndex].color) continue;
                        defPtr = &d; break;
                    }
                    if (defPtr) {
            ...
            if (stacked && stackOnIndex >= 0) {
                auto isLogLike = [](PieceType t) {
                    return t == PieceType::StraightLog || t == PieceType::NotchedLog
                        || t == PieceType::TreeTrunkHorizS || t == PieceType::TreeTrunkHorizM
                        || t == PieceType::TreeTrunkHorizL;
                };
                bool belowIsLog = isLogLike(placedPieces[stackOnIndex].type);
                bool placingLog = isLogLike(placingType);
        ...
        if (isPlacing && haveGhostPos) {
            const PieceDef* defPtr = nullptr;
            for (auto& d : pieceDefs) if (d.type==placingType) { if (UsesColorVariants(placingType) && d.defaultColor!=placingColor) continue; defPtr=&d; break; }
