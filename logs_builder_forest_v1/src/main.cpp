...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (hasController && !inventory.isOpen) {
            ...
            if (fabsf(rightStick.x) > deadzone || fabsf(rightStick.y) > deadzone) {
                camera.position.x += rightStick.x * 0.1f;
                camera.position.z += rightStick.y * 0.1f;
                camera.target.x += rightStick.x * 0.1f;
                camera.target.z += rightStick.y * 0.1f;
            }
        }
        ...
        ImGui::Separator();
        ImGui::Text("Right Stick: Move camera");
