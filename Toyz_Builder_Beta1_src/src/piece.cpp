...
void DrawPlacedPiece(const PlacedPiece& piece, const std::vector<PieceDef>& defs) {
    ...
    if (piece.type == PieceType::StraightLog || piece.type == PieceType::NotchedLog) {
        tint = WHITE; // bark texture shows through
    ...
    Color shadedTint = {
        (unsigned char)(tint.r * (0.85f + heightFactor * 0.15f)),
        (unsigned char)(tint.g * (0.85f + heightFactor * 0.15f)),
        (unsigned char)(tint.b * (0.85f + heightFactor * 0.15f)),
        tint.a
    };
